...
`site-research-astro/contents/` と `site-public/` 配下の本文はサイトに出る読者向けテキスト。次の2つは書かない:

1. **実装用語**:「md」「markdown」「frontmatter」など。
   - ファイル名やリンクパス内の `.md` 拡張子はOK
2. **そのページについてのメタ説明**:「このページの目的」「このページの役割」「運用方法」など、本文ではなくページ自体について語る節。
   - そういう運用ルールは CLAUDE.md（リポジトリ各所のディレクトリにある運用メモ）に書く
   - どうしても読者にページの趣旨を伝えたい場合は、見出しを置かずに 1〜2 行のリード文として導入に溶かす
