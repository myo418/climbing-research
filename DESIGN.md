...
各エントリー（書籍・チャンネル・人物・場所・用語 etc.）を書くとき、次の3点を優先順位どおりに揃える:

1. **画像** — 1枚以上入れる。複数あれば複数入れてよい。なければ省く。
   - 書籍 → 表紙（Amazon CDN から取得）
   - YouTubeチャンネル → 代表動画のサムネイル
   - 人物 → 公式プロフィール写真、競技・登攀写真など複数可
   - 岩場・場所 → 全景・課題部分・アクセスなど用途別に複数可
2. **動画埋め込み** — 関連動画があれば `<VideoEmbed>` でエンベッドする。複数あれば複数並べてよい。一覧ページでは代表動画を1〜2本、個別ページでは制限なし。
3. **リンク** — 公式サイト・Amazon・SNS・購入先など参照元があれば必ず入れる。外部リンクは `<SourceLink>` を使う。

「書けることは書いてあるが、絵がなく文字だけ」の状態を避ける。
